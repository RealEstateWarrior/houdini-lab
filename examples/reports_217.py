...
LAB = {"b-3": "曲げの硬さ 0.001", "b-1": "0.1（既定）", "b0": "1", "b0s3": "3", "b1": "10", "b3": "1000"}
...
def main():
    ...
    font = _font(17)
    w, h = 560, 360
    sheet = Image.new("RGB", (w * 3, (h + 30) * 2), (236, 236, 238))
    ...
    payload = {
        ...
        "summary":
            ...
            f"実験206・211 と同じ机と布（{d['grid_rows']}×{d['grid_cols']}）を {d['last']} フレーム落とし、曲げの硬さだけを 0.001・0.1・1・3・10・1000 にした（Stiffness と × の指数で）。"
            ...
            f"**0.1 と 10 の間では、なめらかに硬くなる。**張り出しは 1 で {out('b0'):.1f} cm、3 で {out('b0s3'):.1f} cm。"
            f"裾の高さは 1 で {hem('b0'):.3f} m（既定とほぼ同じ）、3 で {hem('b0s3'):.3f} m。角のいちばん低い点は 1 ですでに {t['b0']['lowest']:.3f} m まで上がった。\n\n"
            "**決め方: この大きさの布（1.9 × 1.45 m、66×84）では、0.1〜10 の間で見せ分ける。**0.1 より下げても見た目は変わらない。10 より上げてもほぼ同じ。",
        "graph": "", "graph_image": "",
        "comparisons": [
            {"label": "曲げの硬さを変えたテーブルクロス（72 フレーム目）",
             "images": [{"path": "217_grid.png", "caption": "上の段 0.001・0.1・1、下の段 3・10・1000。0.001 と 0.1 はほぼ同じ。硬くするほど布が張って机から突き出す。"}],
        ...
        "notes": [
            ...
            f"<strong>見せ分けは 0.1〜10 の間。</strong>1 で張り出し {out('b0'):.0f} cm、3 で {out('b0s3'):.0f} cm と、なめらかに変わる。",
        ],
        "next": [],
    }
