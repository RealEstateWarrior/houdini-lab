...
def main():
    ...
    cover = g.node("attribwrangle", "icing_area", [puffy], snippet=(
        ...
        "float edge = chf('edge') + (noise(c * 25) - 0.5) * chf('wobble');\n"
        "if (n.y < edge) removeprim(0, @primnum, 1);"))
    cover.parm("class").set(1)
    kit_spare(cover, edge=-0.15, wobble=0.9)
    icing = g.node("polyextrude::2.0", "icing_thickness", [cover], dist=0.004, outputback=1)
    g.step(icing, "アイシングをかける",
           "生地のコピーを <code>attribwrangle</code>（Primitives）で削る。面の向きの上下成分 N.y が <strong>edge = −0.15</strong> より小さい面を消すと、"
           "上側と少し側面まで残る。edge をノイズで上下させる（wobble 0.9）と、縁が垂れたように波打つ。"
           "<code>polyextrude</code> で <strong>4 mm</strong> 押し出して厚みを付ける（Output Back で裏も閉じる）。",
           cap="上半分を覆う、縁の波打ったアイシング。", shading="smooth", ui_parm="edge")
    ...
    grain = g.node("sphere", "sprinkle", type="polymesh", rad=(0.0012, 0.0012, 0.0045), rows=8, cols=12)
    ...
    g.step(sprinkles, "カラースプレーを散らす",
           ...
           "アイシングの厚みぶん持ち上げる。色は6色から選ぶ。細長い <code>sphere</code>（長さ 9 mm）を <code>copytopoints</code> で並べ、"
    ...
    bread = g.mat("dough_mat", basecolor=(0.72, 0.45, 0.22), rough=0.6, sheen=0.4)
    glaze = g.mat("icing_mat", basecolor=(0.95, 0.55, 0.68), rough=0.15, coat=0.6)
    ...
    g.step(final, "材質を当てる",
           "生地は焼き色の茶色で Roughness 0.6・Sheen 0.4。アイシングはピンクで <strong>Roughness 0.15・Coat 0.6</strong>（つや）。"
    ...
    g.hero(final, "Karma で撮った仕上がり。ピンクのアイシングとカラースプレーのドーナツ。", direction=(0.45, 0.75, 1.0),
           key=0.35, rim=0.6, dome=0.25, dome_color=(1.0, 0.97, 0.94), spp=32, margin=1.1, backdrop=(0.3, 0.36, 0.4),
