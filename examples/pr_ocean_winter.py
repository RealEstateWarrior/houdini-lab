...
SKY_R = 20000.0
SUN_POWER = 150000.0
SPECTRA = (("swell", 200, 5, 0.5, 10, 0.5), ("wind_chop", 23, 6, 1.3, -30, 0.7), ("ripples", 1.3, 7, 1.5, 20, 1.6))
...
// 岸へ進む波の列。長さ 26 m・周期 9 秒。横に少し曲げて、まっすぐな線にしない
float L = 26, T = 9;
float bend = 0.8 * noise(set(v@rest.x / 70, 3.1, 0)) - 0.4;
float ph = 2 * M_PI * (s / L + t / T) + bend * 6;
float sb = 34 + 14 * (noise(set(v@rest.x / 45, 7.7, 0)) - 0.5);   // 崩れ始める所（沖から 27〜41 m）
float a = fit(s, sb, 160, 0.75, 0.12);           // 沖から寄るほど高く
if (s < sb) a = 0.75 * fit(s, -2, sb, 0.12, 0.55);   // 崩れたあとは低く
a *= 0.75 + 0.5 * noise(set(v@rest.x / 30, 1.3, t * 0.05));
float c = 0.5 + 0.5 * cos(ph - 0.9 * sin(ph)); // 岸側が切り立つ形
@P.y += a * (pow(c, 2.2) - 0.25) * smooth(-3, 3, s);
// 泡: 崩れた波頭の白い線 ＋ 通ったあとに残るレース状の泡
float u = ph / (2 * M_PI); u -= floor(u);       // 波頭が通ってからの時間（0〜1）
float zone = (1 - smooth(sb - 6, sb + 4, s)) * smooth(-4, 0, s);   // 崩れる所から岸まで
float crest = smooth(0.9, 0.99, c) * zone * smooth(0.3, 0.6, noise(set(v@rest.x / 6, t * 0.15, 2.0)));
float f1, f2; int seed = 0;
vector q = set(v@rest.x, 0, v@rest.z + t * 1.1) * 0.9;   // 泡の模様は岸へ流す
wnoise(q, seed, f1, f2);
float lace = 1 - smooth(0.02, 0.07, f2 - f1);   // 網の目の線
float blot = smooth(0.35, 0.05, f1);            // 丸いかたまり
float patch = smooth(0.42, 0.68, noise(q * 0.12 + set(0, t * 0.03, 0)));
float trail = exp(-u * 3.2) * zone;
float F = crest + (trail * (0.4 + 0.6 * patch) + 0.5 * zone * patch) * max(lace, blot * 0.6);
f@foam = clamp(F, 0, 1);
v@Cd = lerp({{0.006, 0.018, 0.032}}, {{0.85, 0.87, 0.88}}, f@foam);"""
...
def main():
    ...
    g.step(fan, "板を水平線まで広げる",
           "夕暮れ版の 1・2 段目と同じ。<code>grid</code>（Size 1 × 1、Rows 1400・Columns 1600、Center 0.5・0・0.5）を、"
           "<code>attribwrangle</code> でカメラの前の扇形に並べ直す。違いは 2 つ。"
           "広角（焦点距離 22 mm）で撮るので横幅を距離と同じにしたことと、動かす前の位置を <code>rest</code> に覚えておくこと。"
    ...
    specs = [g.node("oceanspectrum", name, gridsize=gs, res=9 if gs > 10 else 8, windspeed=ws, chopscale=chop,
                    winddir=wd, seed=i + 5, ampscale=amp) for i, (name, gs, ws, chop, wd, amp) in enumerate(SPECTRA)]
    mix = g.node("merge", "three_spectra", specs)
    wave = g.node("oceanevaluate::2.0", "move_points", [fan, mix], time="=$T")
    g.step(wave, "沖の波を 3 枚の設計図で動かす",
           "夕暮れ版と同じく、<code>oceanspectrum</code> 3 枚を <code>merge</code> して <code>oceanevaluate</code> に渡す（Time に <code>$T</code>）。"
           "冬の朝は風が少し強く、写真の水面は一面がきらめいていた。きらめきは、細かい波の 1 つ 1 つが太陽を映したものなので、"
           "<strong>さざ波（ripples）を Grid Size 1.3・Speed 7・Chop 1.5・Scale 1.6</strong> と、細かく急にする。"
           "うねりは Grid Size 200・Speed 5・Scale 0.5、風の波は 23・6・0.7。",
    ...
    g.step(surf, "岸で崩れる波と泡を VEX で足す",
           ...
           "<br>・<strong>波の列</strong>: 長さ 26 m・周期 9 秒で岸へ進む。<code>cos(ph - 0.9 * sin(ph))</code> で、岸側が切り立つ形にする。"
           "<br>・<strong>高さ</strong>: 沖 160 m で 0.12 m、崩れる所（沖 27〜41 m、横に揺らす）で 0.75 m。崩れたあとは低くする。"
           "<br>・<strong>泡</strong>: 崩れる所から岸までを泡の範囲にし、波頭の白い線と、波頭が通ったあとに薄れていくレース状の泡（<code>wnoise</code> の網目）を足す。"
           "<br>点の色 <code>Cd</code> を、深い青から泡の白へ混ぜる。",
           cap=f"泡の白い線とレース状の泡（フレーム {FRAME}）。", shading="smooth", bbox=near_box, ui_parm="snippet")
    nrm = g.node("normal", "smooth_normals", [surf])

    def sand(name, z0, z1, rows):
        s = g.node("grid", name, size=(120, z1 - z0), rows=rows, cols=600, t=(0, 0, (z0 + z1) / 2))
        return g.node("attribwrangle", name + "_shape", [s],
                      snippet=f"// 岸へ向かって下がる砂浜（傾き {SLOPE}）に、細かい凸凹\n"
                              f"@P.y = {SLOPE} * (@P.z + {SH:g}) + 0.01 * noise(@P * 3) + 0.03 * noise(@P * 0.2);\n"
                              "v@Cd = {0.03, 0.029, 0.027} * (0.8 + 0.4 * noise(@P * 8));   // 七里ヶ浜の黒っぽい砂")
    wet = g.node("normal", "wet_sand_n", [sand("wet_sand", -13, -3, 300)])
    ...
    g.step(wet, "砂浜を敷く",
           ...
           cap="濡れた砂（波打ち際の手前 4 m と、水の下）。", shading="smooth",
    ...
    water = g.mat("sea_mat", basecolor=(1, 1, 1), basecolor_usePointColor=1, rough=0.04, reflect=1.0, ior=1.33)
    ...
    g.step(final, "材質と太陽の光を置く",
           "<code>principledshader</code> を 3 つ。水は Use Point Color・<strong>Roughness 0.04</strong>・IOR 1.33。"
