...
def main():
    ...
    petal = g.node("attribwrangle", "petal_shape", [blade], snippet=(
        ...
        "float w = sin(PI * pow(u, 0.7)) * 0.5;\n"
        "@P.z *= w * chf('width') * 2;\n"
        "@P.x = u * chf('length');\n"
        "if (u > 0.9) @P.x -= (0.5 - abs(@P.z / (chf('width') + 1e-6))) * 0.004 * (u - 0.9) * 10;   // 先の切れ込み\n"
        "@P.y = 0.25 * chf('length') * pow(abs(@P.z / chf('width')), 2) * 0.3 + 0.15 * chf('length') * u * u;\n"
        "v@Cd = lerp({0.95, 0.5, 0.66}, {1.0, 0.82, 0.88}, u);"))
    kit_spare(petal, width=0.02, length=0.03)
    g.step(petal, "花びらを1枚作る",
           "<code>grid</code> を <code>attribwrangle</code> で丸い花びらにする。付け根から先への位置 u で幅を sin の形にし、"
           "先に小さな切れ込みを入れ、少し反らせる。長さ 3 cm（本物より少し大きめにして、画面で見えるようにする）。"
           "色は付け根が濃いピンク、先が白に近いピンク。",
           cap="桜の花びら1枚（拡大）。", shading="smoothwire",
           bbox=hou.BoundingBox(-0.004, -0.008, -0.022, 0.034, 0.012, 0.022))

    canopy = g.node("grid", "blossom_area", size=(3.0, 1.2), rows=2, cols=2, t=(0, 2.4, -0.3))
    ...
    g.step(parts, "粒を生んで、ゆっくり落とす",
           "高さ 2.4 m に広い板（3 × 1.2 m）を置き、<code>dopnet</code> の中の <code>popsource</code> で、その面から "
           "<strong>1 秒に 90 個</strong>の粒を生む（Emission Type は Surface）。はじめの速さは横 0.3・下 0.2 m/秒に、ばらつきを付ける。"
    ...
    g.hero(final, f"Karma で撮った仕上がり（フレーム {HERO_F}）。風に流されて舞い散る桜の花びら。", frame=HERO_F,
           direction=(0.25, 0.15, 1.0), key=3.0, rim=5.0, dome=0.6, dome_color=(0.85, 0.9, 1.0), spp=48, margin=1.0,
           backdrop=(0.5, 0.62, 0.78), backdrop_reflect=0.0, bbox=hou.BoundingBox(-0.5, 0.5, -0.3, 0.9, 1.5, 0.3))
