...
def main():
    ...
    payload = {
        "title": f"焚き火を Karma で撮るとき、Volume Step Rate を 0.25 から下げても速くならず、1 に上げると遅い — 1920×1080 では 1 が {s('v03_big_s1.0'):.0f} 秒、0.125 が {s('v03_big_s0.125'):.0f} 秒。Pyro を粗くしても撮る時間は同じで、炎の形だけ変わる",
        "summary":
            "**課題: 焚き火を何百フレームも撮るなら、1 枚を短くしたい。Karma の Volume Step Rate（ボリュームの中を進む細かさ。既定 0.25）を変えると、どれだけ速くなり、見た目はどれだけ変わるか。Pyro を粗い升で回した炎は、撮っても見劣りしないか。**\n\n"
            ...
            f"**Step Rate を変えても、ほとんど速くならない。**960×540 で 1・0.5・0.25・0.125 は {s('v03_s1.0'):.1f}・{s('v03_s0.5'):.1f}・{s('v03_s0.25'):.1f}・{s('v03_s0.125'):.1f} 秒。"
            f"1920×1080 にすると、1 が {s('v03_big_s1.0'):.1f} 秒、0.125 が {s('v03_big_s0.125'):.1f} 秒で、1 の方が {s('v03_big_s1.0') / s('v03_big_s0.125') - 1:.0%} 遅かった。"
            "**訂正（2026-09-24、実験216）: はじめ、この記事では「1 は粗い」と書いていた。逆で、Volume Step Rate は大きいほど細かく進む。**"
            "実験216 で一様な煙の箱を撮ると、1 では光の吸収の式とぴったり合い、0.25 では濃い煙が式より少し明るく、0.05 ではほぼ素通りになった。"
            "1 が遅かったのは、細かく進むためと考えられる。\n\n"
            ...
            "**決め方: 焚き火なら Volume Step Rate は既定の 0.25 のまま。**撮る時間を縮めたいなら、サンプル数とノイズ除去（実験205）や画の大きさで決める。Voxel Size は、撮る時間ではなく炎の形で決める。",
        "graph": "", "graph_image": "",
        "comparisons": [
        ...
        "notes": [
            f"<strong>焚き火なら Volume Step Rate は既定の 0.25 のままでよい。</strong>0.125 に下げても速くならず、1 に上げると 1920×1080 で {s('v03_big_s1.0') / s('v03_big_s0.125') - 1:.0%} 遅い。見た目の差は見分けられない。",
            "<strong>訂正: Volume Step Rate は大きいほど細かい。</strong>はじめ「1 は粗い」と書いていた（実験216 で確かめて直した）。",
