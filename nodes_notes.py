...
NOTES = {
    ...
    "scatter": ("既定の Relax Iterations のままだと、点の一部が板の縁の上にぴったり乗る（1,000点中52点）（実験123）。 density 属性は点を面積あたりの比で配り、Force Total Count を切ると数は Density Scale × 面積のあたりで揺れる（実験155）。Force Total Count に 1000 万を入れても、点は 100 万個しか出なかった（21.0.700）。それより多い点は pointgenerate で作る（実験228）。", ["123", "155", "228"]),
    ...
    "vellumconstraints": ("Cloth は網を三角形にしてから、辺ごとに伸び（distance）、内側の辺ごとに曲げ（bend）の拘束を作る。四角形で渡しても三角形で渡しても本数は同じ（実験163）。Pressure の Rest Length Scale は、ふくらんだ体積の倍率そのもの。しずく形の風船で 1.5・2・3・4・6 にすると体積は 1.50・2.00・3.00・4.00・6.00 倍になり、2 フレームで落ち着いた。2 倍を超えるとしずく形が消えて球になった（実験237）。", ["163", "237"]),
        "rbdmaterialfracture": ("Glass の Radial Crack Number（放射状のひび、既定 20）を 80 に上げると、破片は 119 → 474 個。同じつながりの強さ 0.8 では割れにくくなり（動いた破片 17% → 5%）、強さを 0.6 前後に下げると同じくらい割れた。ただし割れるか割れないかの境目の強さでは、同じ設定でも回すたびに割れ方が揺れた（0.6 で動いた破片 54〜116 個）。1 スレッド（HOUDINI_MAXTHREADS=1）なら毎回同じになるが約 3 倍遅い（実験246）。計算は割る・Bullet とも 3 秒以内（実験226）。", ["226"]),
"vellumsolver": ("既定（Constraint Iterations 100・Substeps 1）では、ぶら下げた布が 2.3% 伸びる。伸びは Iterations を上げる方が安く減る（400 回で 0.2%・時間 1.5 倍）。Stretch Stiffness は Substeps 1 では 10^4 と 10^10 で差が出ない（実験170・177）。机に落とすテーブルクロスでは、Substeps の方が時間の割によく効いた。Substeps 1→5 で伸び 2.8%→0.10%、Iterations 400 より Substeps 10 の方が短い時間で伸びが小さい。布の目を 4 倍にすると約 4 倍伸びる（実験224）。", ["170", "177", "224"]),
    ...
    "heightfield_erode": ("土の量（高さの合計）を保たない。40 フレームで平均の高さが約 5 下がり、sediment・debris を足しても元に戻らない（実験169）。Freeze at Frame（既定 5）までで止まる。1000 m 四方・500×500 の山で、5 フレームで谷筋が刻まれ、20 で谷の出口に扇状地、100 で谷が埋まって高さのばらつき 29 → 18 m。時間は 100 フレームでも 3 秒（実験227）。谷の細かさは升（Grid Spacing）ではなく Erosion Feature Size（既定 10 m）で決まる。升 2 m と 1 m はほぼ同じ絵・同じ時間で、Feature Size を 2.5 m にすると細かい谷が刻まれた（実験245）。", ["169", "227", "245"]),
}
...
MORE = {
    ...
    "karma": ("被写界深度（Enable Depth of Field）とモーションブラーを入れても、960×540・16 サンプルで 25 秒 → 27 秒ほどしか延びなかった。Primary Samples を 16 → 256 にすると 3.3 倍の時間でも、既定の Noise Level 0.01 のままではざらつきは減らなかった。Denoiser を oidn にすると、ほぼ同じ時間でいちばん滑らかだった（実験205）。Volume Step Rate は大きいほど細かい。焚き火では既定 0.25 のままでよい（0.125 に下げても速くならず、1 に上げると 1920×1080 で 23% 遅い。実験207）。濃い煙を正しい濃さで撮るなら上げる（一様な煙の箱で、1 は光の吸収の式とぴったり、0.25 は Density Scale 4 で 12% 明るい。実験216）。光を追う回数の目安（実験234〜236・238）: 窓の光の部屋は Diffuse Limit を既定 1 → 4 以上（明るさ 72% → 91%）。雲は Volume Limit を上げるほど明るく、16 でも落ち着かない（時間 13 → 378 秒）。氷の入ったグラスは Refraction Limit を既定 4 → 8 で氷が透明に。Color Limit（既定 10）は太陽の映り込みを切り詰めていて、上げるとざらつきが増える。速く回る物を SOP で動かすなら Geometry Time Samples を 8 に（実験230）。Rendering Engine を XPU にすると 7〜14 倍速いが、形に点の色 Cd があると principledshader の色が Cd に置き換わる（Use Point Color を切っていても）。MaterialX の材質なら CPU と同じ色（実験232・242・243）。", ["205", "207", "230", "232", "234", "235", "236", "238", "242", "243"]),
    ...
    "pyrosolver": ("出力のボリュームは density・temperature・flame と vel.x・vel.y・vel.z の 6 つ。そのまま .bgeo.sc に書くと、Voxel Size 0.04 の焚き火で 1 フレーム約 3 MB、その 9 割以上が vel だった（実験204）。Turbulence は、Use Control Field が入っていると焚き火でまったく効かず（揺らぎなしと全フレーム同じ）、切ると炎が 1.21 → 1.31 m になった。作った直後の Use Control Field は「入り」だが、revertToDefaults で戻る値は「切り」なので、台本で戻すときは注意（実験220）。Dissipation は、火元が density を出していないと効かない（Source Burn は burn・temperature だけ）。火元に density を足すと、煙の量は Dissipation にほぼ反比例した（0.05・0.1・0.2・0.4 で 62・30・14・7。実験244）。", ["204", "220", "244"]),
}
