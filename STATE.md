...
- 実験 001〜201 完了（200 から制作寄り。これからは実践を増やすほうへ重心を移す）。振り返り点検は 180（151〜179 を全部流し直し、時間以外は全部一致。examples/180_audit.py）まで済み、次は 210
  （点検の番号は 030・060・090・121・150・180）。Artifact は 150 時点のまま（2026-09-23 にユーザーが force での出し直しを許可済み。次のセッションの最初にやる）。次の実践候補: カーテン・風船・ゼリー・海の波・雪
