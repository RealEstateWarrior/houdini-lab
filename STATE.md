...
最終更新: 2026-09-24 夕方（実験217・実践71本・ノード388件・用語453語・VEX 解説ページ）
...
- **2026-09-24 午後（ユーザー指示「実践を想定した実験を17時まで・用語とノードも・VEX 解説ページ」）**:
  ...
  - **点検 210（181〜209）済み**: 22 本は完全一致。200・205 は足した項目、204・209 はファイルの大きさの揺れ（最大 0.6%）、181・208 は時間の項目。
    201 は台本が 202 の build（焚き火の設定）を借りていて合わなかった → 既定値に戻すよう直した。**それでも Turbulence を Use Control Field 入りで効かせた 2 条件は合わない**
    （記事に注意書き。やることメモ open201）。次の点検は 240
  - 215（草を何本まで Karma で撮れるか）は途中で止めた（1 万本 44 秒、10 万本は 10 分超）。やることメモ mine215
...
- 実験 001〜217 完了（215 だけ途中。200 から制作寄り。これからは実践を増やすほうへ重心を移す）。振り返り点検は 210（examples/210_audit.py）まで済み、次は 240
