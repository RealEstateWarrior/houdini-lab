...
最終更新: 2026-09-24 午後（実験209・実践71本・ノード388件・用語446語・VEX 解説ページ）
...
- 実験 001〜209 完了（200 から制作寄り。これからは実践を増やすほうへ重心を移す）。振り返り点検は 180（151〜179 を全部流し直し、時間以外は全部一致。examples/180_audit.py）まで済み、次は 210
  （点検の番号は 030・060・090・121・150・180）。Artifact（サイトの写し）は 150 時点のまま。**出し直しは 2026-09-23 にやめた**（本体は GitHub Pages。ユーザーに頼まれたときだけ出す）
